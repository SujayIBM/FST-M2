-- Load the input data
inputDialogues4 = LOAD 'hdfs:///user/sujbhatt/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/sujbhatt/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/sujbhatt/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/sujbhatt/outputs;

-- Store result in hdfs
STORE namesOrdered INTO 'hdfs:///user/sujbhatt/outputs' USING PigStorage('\t');
