...
inputFile1 = LOAD 'hdfs:///user/sujbhatt/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray,line:chararray);
inputFile2 = LOAD 'hdfs:///user/sujbhatt/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray,line:chararray);
inputFile3 = LOAD 'hdfs:///user/sujbhatt/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray,line:chararray);
...
STORE ordered INTO 'hdfs:///user/sujbhatt/output' USING PigStorage('\t');
